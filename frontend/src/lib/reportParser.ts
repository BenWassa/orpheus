...
function normalizeCoOccurrences(value: unknown): CoOccurrence[] {
  if (!Array.isArray(value)) return [];
  return value.flatMap((item) => {
...
function normalizeCoOccurrence(item: unknown): CoOccurrence | null {
  ...
  return {
    ...
    lift: typeof source.lift === 'number' && Number.isFinite(source.lift) ? source.lift : undefined,
...
export function parseOrpheusReport(input: unknown): OrpheusReport {
  ...
  const report: OrpheusReport = {
    ...
    co_occurrences: normalizeCoOccurrences(root.co_occurrences),
