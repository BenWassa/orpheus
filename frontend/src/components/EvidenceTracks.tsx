        ...
        <div className="track-grid">
          {tracks.map((track) => (
            <article className="track-card" key={track.uri}>
              ...
              <p>{[track.artist, track.album].filter(Boolean).join(', ') || track.uri}</p>
              {track.play_count && <small>{track.play_count} plays in this window</small>}
              {!track.play_count && typeof track.weight === 'number' && <small>Report weight {track.weight.toFixed(2)}</small>}
